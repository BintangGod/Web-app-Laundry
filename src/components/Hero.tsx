...
export default function Hero() {
  ...
  return (
    <section
      id="home"
      className="relative min-h-[100dvh] pt-24 pb-16 flex items-center justify-center overflow-hidden"
    >
      {/* Background Decorative Amber Glow */}
      ...
      <div className="absolute bottom-0 right-0 w-1/3 h-1/3 bg-primary/3 rounded-full blur-[120px] pointer-events-none" />

      {/* Grid Pattern Accent - subtle on dark */}
      <div className="absolute inset-0 bg-[linear-gradient(to_right,rgba(255,255,255,0.02)_1px,transparent_1px),linear-gradient(to_bottom,rgba(255,255,255,0.02)_1px,transparent_1px)] bg-[size:4rem_4rem] [mask-image:radial-gradient(ellipse_60%_50%_at_50%_0%,#000_70%,transparent_100%)] pointer-events-none" />

      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10">
        {/* Modern Pill Badge (Green) */}
        <ScrollReveal delay={0.1}>
          <div className="inline-flex items-center space-x-2 px-4 py-2 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-xs font-semibold tracking-wider uppercase mb-8">
        ...
        {/* Brand Main Title - Buzzworthy-style large display */}
        <ScrollReveal delay={0.2}>
          <h1 className="text-6xl sm:text-7xl md:text-8xl lg:text-9xl font-extrabold tracking-tighter leading-[0.9] mb-6">
            <span className="block text-white">Bintang</span>
        ...
        <ScrollReveal delay={0.3}>
          <p className="text-lg sm:text-xl text-text-muted font-medium max-w-2xl mx-auto mb-4">
        ...
        <ScrollReveal delay={0.4}>
          <p className="text-sm sm:text-base text-text-dim max-w-xl mx-auto mb-10 flex items-center justify-center space-x-2">
        ...
        <ScrollReveal delay={0.5}>
          <div className="flex flex-col sm:flex-row justify-center items-center gap-4">
            ...
            {/* Secondary CTA (Price List Scroll) - Ghost with amber border */}
            <a
              href="#price-list"
              className="w-full sm:w-auto inline-flex items-center justify-center px-8 py-4 rounded-2xl text-base font-semibold text-text-muted hover:text-primary bg-transparent hover:bg-primary/5 border border-white/10 hover:border-primary/30 transition-all duration-300 hover:-translate-y-1 active:translate-y-0"
            >
              Lihat Daftar Harga
            </a>
